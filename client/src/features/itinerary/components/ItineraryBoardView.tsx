...
import { Heart, Calendar, CreditCard, Trash2, MapPin, Loader2, ArrowRight, Ban } from 'lucide-react';
import { Link } from 'react-router-dom';
import { ItineraryBoard } from '../../../components/ItineraryBoard';
import { itineraryApi } from '../services/itineraryApi';

export function ItineraryBoardView() {
  ...
  const fetchFavoritesAndItinerary = async () => {
    setLoading(true);
    try {
      const [favData, itinData] = await Promise.all([
        itineraryApi.getFavorites(),
        itineraryApi.getItinerary()
      ]);
  ...
  const fetchFavorites = async () => {
    setLoading(true);
    try {
      const data = await itineraryApi.getFavorites();
      setFavorites(data || []);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const data = await itineraryApi.getUserBookings();
      setBookings(data || []);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const cancelBooking = async (id: string) => {
    try {
      await itineraryApi.cancelBooking(id);
  ...
  const removeFavorite = async (itemId: string, itemModel: "Heritage" | "Culture" | "Hotel" | "Vehicle") => {
    try {
      await itineraryApi.toggleFavorite(itemId, itemModel);
    ...
    <div className="w-full bg-gray-50/50 min-h-screen pb-24">
      ...
      <div className="relative bg-gradient-to-r from-gray-950 via-gray-900 to-green-950 text-white py-16 sm:py-20 mb-12 rounded-b-3xl sm:rounded-b-[2.5rem] overflow-hidden shadow-2xl">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          >
            My <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-green-300 to-teal-200">Trips</span>
          </motion.h1>
          ...
          >
            Manage your saved heritage destinations, custom travel itineraries, and verified bookings all in one place.
          </motion.p>
        </div>
      </div>
...
export default ItineraryBoardView;
