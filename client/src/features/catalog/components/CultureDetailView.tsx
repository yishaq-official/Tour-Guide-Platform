...
export function CultureDetailView({ id }: { id?: string }) {
  ...
  const toggleFavorite = async () => {
    ...
    try {
      await itineraryApi.toggleFavorite(id, "Culture");
  ...
  if (error) {
    return (
      <div className="text-center py-32 bg-gray-50 min-h-screen">
        ...
        <div className="mt-4">
          <Link to="/explore?tab=cultures" className="text-green-600 hover:underline">
            Return to Explore
          </Link>
        </div>
      </div>
  ...
  if (!culture) {
    return (
      <div className="text-center py-32 bg-gray-50 min-h-screen">
        ...
        <Link to="/explore?tab=cultures" className="text-green-600 mt-6 inline-block hover:underline text-lg">
          Return to Gallery
        </Link>
      </div>
    );
  ...
  return (
    <div className="w-full bg-white pb-24">
      ...
      <div className="relative h-[65vh] md:h-[80vh] w-full bg-gray-900">
        <img src={culture.image} alt={culture.name} className="w-full h-full object-cover opacity-90" />
            ...
            {session && (
              <button
                ...
                className={`p-3 rounded-full backdrop-blur-md border border-white/20 transition-all shadow-lg ${
                  isFavorite ? "bg-red-500/90 text-white" : "bg-black/40 text-white/80 hover:bg-black/60 hover:text-white"
                }`}
              >
        ...
        <div className="bg-gradient-to-r from-gray-900 via-gray-800 to-green-950 text-white rounded-3xl p-6 sm:p-8 shadow-xl border border-gray-800 mb-12 flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <button
            ...
            className={`w-full sm:w-auto px-6 py-3.5 rounded-xl font-black text-xs uppercase tracking-wider transition-all shadow-lg shrink-0 flex items-center justify-center gap-2 ${
              isPlayingAudio ? "bg-red-500 hover:bg-red-600 text-white" : "bg-green-600 hover:bg-green-700 text-white"
            }`}
          >
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-[11px] font-extrabold text-emerald-800 uppercase tracking-wider mb-1">{key}</h4>
                      <p
